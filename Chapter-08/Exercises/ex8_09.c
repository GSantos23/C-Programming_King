/* 
    Write a program that generates a "random walk" across 10 X 10 array. The array
    ...
    the order visited. Here's an example of the desired output:
    ...
    four possible values for the remiander --0, 1, 2,and 3 -- indicating the direction
    of the next move. Before performing a move, check that (a) it won't go outside the
    array, and (b) it doesn't take us to an element that already has a letter assigned.
    If either condition is violated, try moving in another direction. If all four directions
    are blocked, the program must terminate. Here's an example of premature termination:
...
int main(void)
{
    ...
    while (randArry[r][c] != 'Z' && (occupied[0] == false || occupied[1] == false || occupied[2] == false || occupied[3] == false))
    {
        numGen = rand() % 4;
