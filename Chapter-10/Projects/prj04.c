/*
 * Modify the poker.c program form Section 10.5 by having it recognize an
 * additional category "royal flush" (ace, king, queen, ten of the same suit).
 * A royal flush ranks higher than all the other hands.
*/
...
/******************************************************************************
* analyze_hand: Determines whether the hand contains a royal flush, straight, 
* 				a flush, four-of-a-kind, and/or three-of-a-kind; 
* 				determines the number of pairs; stores the results into the 
...
void analyze_hand(void)
{
    ...
    // source: https://github.com/auwsmit/cpama2/blob/master/Ch10/ProgrammingProjects/p4.c
    if (flush == true)
        royal_flush = true;
...
void print_result(void)
{//flush && royal_flush
    if (royal_flush && flush)		printf("Royal flush");
